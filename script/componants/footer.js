...
export default class Footer extends ManageDom {
  ...
  render() {
    ...
    const salesAndSocialDiv = this.createMarkup("div", "", footer, [
      {
        style:
          "display: flex; align-items: flex-start; flex-direction: column; justify-content: center; padding: 0;",
      },
      { class: "salesAndSocialDiv" },
    ]);
    ...
    const salePointsTitle = this.createMarkup(
      "ul",
      "Nos points de vente",
      salesDiv,
      [
        {
          style:
            "font-size: 18px; font-weight: bold; color: white; text-decofation: none; list-style: none; padding: 0; margin: 0; margin-top: 20px; ",
        },
        { class: "title salePointsTitle" },
      ]
    );
    ...
    const salonSalePoint = this.createMarkup(
      "li",
      "Salon-de-Provence (13300)",
      salesDiv,
      [{ style: "font-size: 12px; color: white; list-style: none;" }]
    );
    const saintSalePoint = this.createMarkup(
      "li",
      "Saint-Nazaire(44600)",
      salesDiv,
      [{ style: "font-size: 12px; color: white; list-style: none;" }]
    ...
    const followUs = this.createMarkup("p", "Nous suivre", salesDiv, [
      {
        style:
          "font-size: 18px; font-weight: bold; color: white; list-style: none; margin: 0; margin-top: 20px;",
      },
      { class: "title follow-us" },
    ]);
    ...
    const dailymotionLink = this.createMarkup("a", "", socialDiv, [
      { href: "https://www.dailymotion.com/fr/" },
    ]);
    const dailymotionImg = this.createMarkup("img", "", dailymotionLink, [
      { src: "../../assets/imgs/dailymotion2.png" },
      { width: "40" },
      { height: "auto" },
      { class: "img" },
    ]);
    ...
    const contactForm = this.createMarkup(
      "form",
      "Nous contacter",
      contactDiv,
      [
        {
          style:
            "display: flex; flex-direction: column; font-size: 18px; font-weight: bold; color: white; list-style: none; margin: 0; margin-top: 20px;",
        },
        { class: "contactForm" },
        { class: "title" },
      ]
    ...
    const nameDiv = this.createMarkup("div", "", contactForm, [
      {
        style:
          "display: flex; flex-direction: row; margin-bottom: 10px; margin-top: 20px;",
      },
      { class: "nameDiv" },
    ]);
    ...
    const firstName = this.createMarkup("input", "", nameDiv, [
      ...
      {
        style:
          "font-weight: bold;border-radius: 5px; margin-right: 10px; width: 50%;",
      },
      { class: "input-footer firstName" },
    ]);
    ...
    const lastName = this.createMarkup("input", "", nameDiv, [
      ...
      { required: "required" },
      { style: "border-radius: 5px; font-weight: bold; width: 50%;" },
      { class: "input-footer lastName" },
    ]);
    ...
    const select = this.createMarkup("select", "Selectionnez", contactForm, [
      ...
      {
        style:
          "font-weight: bold;border-radius: 5px; margin-bottom: 10px; color: grey",
      },
      { class: "input-footer" },
    ]);

    const selectInstruction = this.createMarkup(
      "option",
      "Selectionnez",
      select,
    ...
    const send = this.createMarkup("input", "Envoyer", contactForm, [
      ...
      {
        style:
          "cursor: pointer; font-weight: bold; border: none; border-radius: 5px; height: 25px; font-size: 12px; color: white; background-color: #cc0000; text-align: center; items-align: center;",
      },
      { class: "input-footer" },
    ]);
